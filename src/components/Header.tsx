...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <>
      <header className="h-14 bg-editor-panel border-b border-editor-panelBorder px-3 sm:px-4 flex items-center justify-between select-none relative z-30 shrink-0">
        {/* Brand & Project Info */}
        <div className="flex items-center gap-2 min-w-0">
          <div className="flex items-center gap-1.5 sm:gap-2 bg-blue-600/20 text-blue-400 px-2 sm:px-2.5 py-1.5 rounded-md border border-blue-500/30 shrink-0">
          ...
          <span className="text-[10px] sm:text-xs bg-slate-800 text-slate-400 px-1.5 sm:px-2 py-0.5 rounded border border-slate-700 hidden sm:inline-block shrink-0">
            Project Hail Mary
          </span>

          <div className="h-4 w-[1px] bg-slate-700 mx-0.5 sm:mx-1 hidden sm:block shrink-0" />
          ...
          <div className="flex items-center gap-1 min-w-0">
            <input
              ...
              className="bg-transparent hover:bg-editor-surface focus:bg-editor-surface border border-transparent focus:border-editor-panelBorder rounded px-1.5 sm:px-2 py-1 text-xs sm:text-sm font-medium text-slate-200 focus:outline-none transition-colors w-24 sm:w-40 md:max-w-[200px] truncate"
              title="Click to rename project"
            />
            {isDirty && (
              <span className="text-[9px] sm:text-[10px] bg-amber-950 text-amber-400 border border-amber-800 px-1 sm:px-1.5 py-0.2 rounded font-mono shrink-0">
                Unsaved
              </span>
            )}
          </div>
        ...
        <div className="flex md:hidden items-center gap-1.5">
          {unlinkedCount > 0 && onOpenRelinkModal && (
            <button
              onClick={onOpenRelinkModal}
              className="flex items-center gap-1 px-2 py-1.5 text-[11px] font-semibold bg-amber-950/80 text-amber-300 rounded border border-amber-700/60 animate-pulse"
          ...
          <button
            onClick={handleTriggerRender}
            className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold bg-blue-600 hover:bg-blue-500 text-white rounded shadow-sm"
          ...
          <button
            ...
            className="p-2 text-slate-300 hover:text-white hover:bg-editor-surface rounded-md transition-colors min-w-[40px] min-h-[40px] flex items-center justify-center"
            title="More Options"
          >
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-x-0 top-14 bg-editor-panel/95 backdrop-blur-md border-b border-editor-panelBorder z-40 p-4 shadow-2xl space-y-3 animate-in fade-in slide-in-from-top-2 duration-150 select-none">
          {/* Status specs */}
